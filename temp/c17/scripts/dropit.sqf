...
_Marker = createMarker ["Drop Zone", _orgitem];
_Marker setMarkerShape "ICON";
_Marker setMarkerType "mil_marker";
_Marker setMarkerText "Drop zone";
_flare = "F_40mm_white" createVehicle [_pos select 0,_pos select 1,_z];
...
_flare setpos [_pos select 0, _pos select 1, _z];
...
if (("smoke" in _this) || ("SMOKE" in _this)) then {
  ...
  _ps setParticleRandom [0, [0.25, 0.25, 0], [0.2, 0.2, 0], 0, 0.25, [0, 0, 0, 0.1], 0, 0];
	_ps setParticleParams [["\globemaster_c17\data\RocketSmoke", 8, 3, 1], "", "Billboard", 1, 10, [0, 0, 0], [0, 0, 1], 0, 10, 10, 0.1, [2, 6, 12], [[1, 1, 1, 0.7], [1, 1, 1, 0.15], [1, 1, 1, 0]], [0.125], 1, 0, "", "", _orgitem];
